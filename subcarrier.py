...
if line==0:
  ...
  try: # empty table
     ...
     from data.`download` where ((data.`download`.`agency` <>'%s' ) and (data.`download`.`agency` <>'%s' ) \
     and (data.`download`.`agency` <> '%s') and (data.`download`.`YM` = '%s'))" % (telco1,telco2,telco3,yandm) )
  except mysql.Error, e:
