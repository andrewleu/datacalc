...
while { $finish !=1} {
	 ...
	 if { $query =="" } {
	 	 ...
	 	 set init_time [clock seconds]
	 	  ...
	 	  if {$counter <3 } {  puts "$counter $query $init_time download" }
	 	  incr counter
 ...
 foreach code {"110000" "120000" "310000" "500000"} { 
     ...
     mysqlexec $mysql_handler "begin"
     set query [mysqlsel $mysql_handler "select `name`,`known_as`,`date`  from region where date!='$date'
 ...
 #dianxin idle
  ...
  if { [expr {$ydsample+$ltsample+$dxsample}] } {
...
set num [mysqlsel $mysql_handler "select count(id) from region where level=2 and 
    pid !=110000 and pid!=120000 and pid!=310000 and pid!=500000"  -list]
while {  [mysqlsel $mysql_handler "select count(id) from region
         where level=2 and date='$date' and pid !=110000 and 
         pid!=120000 and pid!=310000 and pid!=500000" -list ] != $num } {
        after 15000}
set finish  0
while {$finish!=1}   {
      mysqlexec $mysql_handler "begin"
    ...
    set totalsample [expr {$ydsample+$ltsample+$dxsample}]
    if { $totalsample !=0 } { 
 ...
 foreach code {"810000" "820000" "710000"} {  ;#台湾 香港 澳门
     set waiting [expr {int(rand()*1000)}]
     after $waiting 
     mysqlexec $mysql_handler "begin"
     set query [mysqlsel $mysql_handler "select `name`,`known_as`,`date`  from region where 
      id='$code' and date !='$date' limit 1 for update" -list]
     if {$query !=""} {
     mysqlexec $mysql_handler "update region set date='$date' where id = '$code' " 
     mysqlexec $mysql_handler "commit"
     set addr [lindex $query 0 0] 
     set knownas [lindex $query 0 1]
     set stat1 [mysqlsel $raw_data "select avg(avi), count(avi) from download where YM='$YM' and day=$day and addr like '$knownas%'" -list]
     set stat1 [string map {"{}" "0"} $stat1]
     set average [lindex $stat1 0 0]
     #puts $addr, $knownas, $average
     mysqlexec $mysql_handler "insert into speed_daily (addr, code,date,degree,average)
       value('$addr','$code','$date',1,'$average')"
     } else {
      mysqlexec $mysql_handler "commit"
    }
} 
 ...
 set end_time [clock format $end_time -format  {%Y-%m-%d %H:%M:%S}]
 puts $end_time
